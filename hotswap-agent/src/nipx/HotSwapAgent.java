...
public class HotSwapAgent {
    ...
    public static boolean DEBUG = Boolean.parseBoolean(
        System.getenv("nipx.agent.debug")
    );
    public static boolean UCP_APPEND = Boolean.parseBoolean(
        System.getProperty("nipx.agent.ucp_append", "true")
    );
    public static int FILE_SHAKE_MS = 1200;
    public static RedefineMode REDEFINE_MODE;
    public static String[] HOTSWAP_BLACKLIST;
    public static boolean RETRANSFORM_LOADED = Boolean.parseBoolean(
        System.getProperty("nipx.agent.retransform_loaded", "false")
    );
    ...
    private static final List<WatcherThread> activeWatchers =
    ...
    private static final Set<Path> pendingChanges = Collections.synchronizedSet(
        new HashSet<>()
    );
    ...
    public static void init(String agentArgs, boolean reinit) {
        ...
        // 解析传入的监控路径 (支持分号或冒号分割)
        Set<Path> newWatchDirs = Arrays.stream(
            agentArgs.split(File.pathSeparator)
        )
    ...
    public static void initConfig() {
        info("DEBUG: " + DEBUG);
        REDEFINE_MODE = RedefineMode.valueOfFail(
            System.getProperty("nipx.agent.redefine_mode", "inject"),
            RedefineMode.inject
        );
        info("Redefine Mode: " + REDEFINE_MODE);
        HOTSWAP_BLACKLIST = System.getProperty(
            "nipx.agent.hotswap_blacklist",
            ""
        ).split(",");
        info("Injection Blacklist: " + String.join(",", HOTSWAP_BLACKLIST));
        ENABLE_HOTSWAP_EVENT = Boolean.parseBoolean(
            System.getProperty("nipx.agent.hotswap_event", "false")
        );
        info("HotSwap Event: " + ENABLE_HOTSWAP_EVENT);
        LAMBDA_ALIGN = Boolean.parseBoolean(
            System.getProperty("nipx.agent.lambda_align", "true")
        );
        info("Lambda Align: " + LAMBDA_ALIGN);
        if (LAMBDA_ALIGN) {
            info(
                "Lambda Alignment ENABLED. Warning: This may cause logical shifts if lambdas are reordered."
            );
        }
    ...
    private static void retransformLoaded(Class<?>[] classes) {
        ...
        info(
            "Found " +
                candidates.size() +
                " classes loaded before Agent start. Retransforming..."
        );
        try {
    ...
    public static volatile ConcurrentHashMap<
        String,
        Class<?>
    > loadedClassesMap = new ConcurrentHashMap<>();

    private static void loadClassSnap(Class<?>[] classes) {
        var newMap = new ConcurrentHashMap<String, Class<?>>(
            (int) (classes.length / 0.75f) + 1
        );
        for (Class<?> c : classes) {
    ...
    private static void processChanges(
            ...
            try {
                ...
                synchronized (fileDiskHashes) {
                    ...
                    if (oldDiskHash != -1 && oldDiskHash == newHash) {
                        if (DEBUG) log(
                            "[SKIP] " + className + " (file hash unchanged)"
                        );
                        skippedCount++;
                        continue;
                    }
                ...
                if (targetClass != null) {
                    ...
                    if (oldBytecode != null) {
                        if (LAMBDA_ALIGN) {
                            newBytecode = LambdaAligner.align(
                                oldBytecode,
                                newBytecode
                            );
                        }

                        ClassDiffUtil.ClassDiff diff = ClassDiffUtil.diff(
                            oldBytecode,
                            newBytecode
                        );
                        ...
                        if (diff.hierarchyChanged) {
                            error(
                                "REJECTED: Class hierarchy change detected for " +
                                    className +
                                    ". JBR/DCEVM does not support changing superclass/interfaces reliably. Please RESTART application."
                            );
                        ...
                        if (!diff.structureChanged()) {
                            if (!isEnhancedHotswapEnabled()) {
                                error(
                                    "STRUCTURAL CHANGE DETECTED! Field/Method structure changed but DCEVM is NOT active."
                                );
                                error(
                                    "This redefine will likely FAIL. Classes: " +
                                        className
                                );
                            } else {
                                log(
                                    "[DCEVM] Structure change detected, proceeding with enhanced redefinition."
                                );
                            }
                        }
                    } else {
                        log(
                            "[WARN] Cannot diff " +
                                className +
                                " (missing old bytecode). Proceeding with redefine."
                        );
                    }
                    ...
                    if (REDEFINE_MODE == RedefineMode.inject) {
                        ...
                        REDEFINE_MODE == RedefineMode.lazy_load && UCP_APPEND
                    ) {
                        ClassLoader loader = findTargetClassLoader(
                            className,
                            path
                        );
        ...
        if (skippedCount > 0) info(
            "Skipped " + skippedCount + " unchanged classes."
        );
        if (injectedCount > 0) info(
            "Injected " + injectedCount + " new classes."
        );
    }
     ...
     * 直接定义类，而不是被动加载
     */
    private static boolean injectNewClass(
        String className,
        Path path,
        byte[] bytes
    ) {
        try {
            ...
            if (loader == null) {
                error(
                    "Could not find a suitable ClassLoader for new class: " +
                        className
                );
                return false;
            }
            ...
            info(
                "[INJECTED] Successfully defined new class: " +
                    className +
                    " into " +
                    loader
            );
            ...
            error(
                "Failed to inject new class: " +
                    className +
                    ". CAUTION: This may cause NoClassDefFoundError.",
                e
            );
            return false;
        }
        ...
        try {
            ...
            for (ClassDefinition def : definitions) {
                try {
                    ...
                    if (DEBUG) log(
                        "[OK] " + def.getDefinitionClass().getName()
                    );
                } catch (Throwable e) {
                                ...
                                while ((entry = zis.getNextEntry()) != null) {
                                    try {
                                        if (
                                            ...
                                            long diskHash = calculateHash(
                                                diskBytes
                                            );

                                            byte[] memBytes = bytecodeCache.get(
                                                cName
                                            );
                                            if (
                                                memBytes != null &&
                                                calculateHash(memBytes) !=
                                                diskHash
                                            ) {
                                                if (DEBUG) log(
                                                    "[INIT-SYNC] " +
                                                        cName +
                                                        " (in jar) is out of sync. Reloading..."
                                                );
    ...
    private static class WatcherThread extends Thread {
                ...
                while (!Thread.currentThread().isInterrupted()) {
                    ...
                    for (WatchEvent<?> event : key.pollEvents()) {
                        ...
                        if (
                            ...
                            if (DEBUG) log(
                                "[WATCH] New directory detected: " + fullPath
                            );
                            ...
                            try (Stream<Path> subFiles = Files.walk(fullPath)) {
                                subFiles
                                    ...
                                    .forEach(p -> {
                                        if (DEBUG) log(
                                            "[WATCH] Found existing file in new dir: " +
                                                p
                                        );
                                ...
                                error(
                                    "Failed to scan new directory: " + fullPath,
                                    e
                                );
                            }
                        }
                    ...
                    if (!key.reset()) {
                        if (DEBUG) log(
                            "WatchKey no longer valid: " + triggeredDir
                        );
                    }
                }
            } catch (IOException e) {
        ...
        private void registerAll(Path startDir) throws IOException {
            if (!Files.exists(startDir)) {
                if (DEBUG) log(
                    "Skipping registration for non-existent directory: " +
                        startDir
                );
                return;
            }
                ...
                stream
                    ...
                    .forEach(dir -> {
                        try {
                            ...
                            if (DEBUG) log(
                                "Failed to register directory: " + dir
                            );
                        }
                    });
            }
    ...
    public enum RedefineMode {
        ...
        public static RedefineMode valueOfFail(
            String inject,
            RedefineMode def
        ) {
            try {
