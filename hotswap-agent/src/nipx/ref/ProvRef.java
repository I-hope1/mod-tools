...
@Tracker
public class ProvRef {

	private volatile Prov<?> fn;
	...
	public ProvRef(Prov<?> fn, Object element, Runnable removeAction) {
		this.fn = fn;
	...
	public static Prov<?> wrap(Element element, Prov<?> original) {
		ProvRef ref = new ProvRef(original, element, element::remove);
		return ref::get;  // ref::run 对应 ProvRef.run()，永远存在
	}
