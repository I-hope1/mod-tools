...
public class UIHookRegistry {
	...
	public static void register(Object obj, String className, String methodName, int lineNumber, int index) {
		...
		if (obj instanceof Cell<?> cell) {
			element = cell.get();
		} else if (obj instanceof Element elem) {
			element = elem;
		} else {
			return;
		}
	...
	public static void updateText(String className, String methodName, int lineNumber, int index, String newText) {
		...
		for (WeakReference<Element> ref : refs) {
			Element elem = ref.get();
			...
			try {
				...
				try {
					...
					try {
						...
						// 对于类似 TextButton，尝试获取其内部 Label 并修改
						try {
							Method getLabel = clazz.getMethod("getLabel");
							Object label    = getLabel.invoke(elem);
							if (label != null) {
								Method setLabelText = label.getClass().getMethod("setText", CharSequence.class);
