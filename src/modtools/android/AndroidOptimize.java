...
public class AndroidOptimize {
	...
	public static class MySpriteBatch extends Batch {
		 ...
		 */
		public MySpriteBatch(int size, Shader defaultShader) {
		...
		protected void prepare(int i) {
			if (requestVertOffset + i >= requestVerts.length) {
				requestVerts = Arrays.copyOf(requestVerts, requestVerts.length << 1);
			}
		}

		protected void expandRequests() {
			Log.info("Expanding requests array to " + requests.length * 7 / 4);
		...
		protected void drawSuper(TextureRegion region, float x, float y, float originX, float originY, float width,
		...
		protected void constructVertices(float[] vertices, int idx, TextureRegion region, float x, float y, float originX,
		                                 float originY, float width, float height, float rotation) {
			// 复用局部变量，帮助JIT优化
			...
			if (Mathf.zero(rotation)) {
				...
				// 预计算旋转后的偏移量
				final float p1x = fx * cos - fy * sin;
				final float p1y = fx * sin + fy * cos;
				final float p2x = fx * cos - fy2 * sin;
				final float p2y = fx * sin + fy2 * cos;
				final float p3x = fx2 * cos - fy2 * sin;
				final float p3y = fx2 * sin + fy2 * cos;
				final float p4x = p1x + (p3x - p2x); // p4 is p1 + (p3 - p2)
				final float p4y = p3y - (p2y - p1y); // p4 is p1 + (p3 - p2)

				final float x1 = p1x + worldOriginX;
				final float y1 = p1y + worldOriginY;
				final float x2 = p2x + worldOriginX;
				final float y2 = p2y + worldOriginY;
				final float x3 = p3x + worldOriginX;
				final float y3 = p3y + worldOriginY;
				final float x4 = p4x + worldOriginX;
				final float y4 = p4y + worldOriginY;
		...
		static class CountingSort {
			...
			static int[] countingSortMap(final int[] arr, final int[] swap, final int end) {
				...
				if (entries.length < unique) {
					...
					int       newSize    = unique * 3 / 2;
					Point2[]  newEntries = new Point2[newSize];
					System.arraycopy(entries, 0, newEntries, 0, prevLength);
					for (int i = prevLength; i < newSize; i++) newEntries[i] = new Point2();
					entries = newEntries; // 原子性替换
				}
				...
				int j = 0;
				if (countEntries.hasNext) {
					// 复用 entry 对象
					for (IntIntMap.Entry entry : countEntries) {
						entries[j++].set(entry.key, entry.value);
					}
				}
				for (int i = j; i < unique; i++) {
					entries[i].set(0, 0); // 清理旧数据，避免干扰排序
				}
