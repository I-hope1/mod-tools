...
public class UnitSpawn extends Content {
	...
	public void _load() {
		...
		ui = new IconWindow(40 * 8, 400, true);
		...
		ui.cont.table(Window.myPane, table -> {
			...
			table.table(t -> {
				...
				teamField = t.field("" + team.id, text -> {
				 ...
				 .valid(val -> NumberHelper.notNegativeInt(val) && toInteger(val) < Team.all.length)
				 .width(100)
				 .get();
			...
			table.table(t -> {
				...
				amountField = t.field("" + amount, text -> amount = (int) toInteger(text))
				 .valid(val -> validNumber(val) && NumberHelper.notNegativeInt(val))
				 .width(100)
				 .get();
			});
		}).growX().row();
