...
public class SettingsContent extends Content {
	...
	public void load() {
		...
		add("jsfunc", new Table() {{
			...
			jsfunc = (Data) settings.get("Jsfunc", () -> new Data(settings, new JsonMap()));
			...
			check("@settings.jsfunc.auto_refresh", jsfunc.getBool("auto_refresh", false), b -> jsfunc.put("auto_refresh", b)).row();
		}});
		add("@mod-tools.others", new Table() {{
			...
			if (Version.number >= 136) {
				SettingsContent.slider(this, "maxSchematicSize", Vars.maxSchematicSize, 500, Vars.maxSchematicSize, 1f, val -> {
