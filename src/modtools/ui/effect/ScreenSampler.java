...
public interface ScreenSampler {
	ScreenSampler instance =
	 ...
	 && ClassUtils.hasMethod(universe.graphic.ScreenSampler.class, "toBuffer")
		? new UnkScreenSampler() : new BaseScreenSampler();

	void getToBuffer(FrameBuffer buffer, boolean clear);
	void setup();
	boolean activity();

	class BaseScreenSampler implements ScreenSampler {
		...
		public void setup() {
			...
			if (!e.getBool("enabled", false)) {
				...
				Events.run(EventType.Trigger.draw, () -> {
					Draw.draw(Layer.min - 0.001f, BaseScreenSampler::beginWorld);
					Draw.draw(Layer.end + 0.001f, BaseScreenSampler::endWorld);
				});

				Events.run(EventType.Trigger.uiDrawBegin, BaseScreenSampler::beginUI);
				Events.run(EventType.Trigger.uiDrawEnd, BaseScreenSampler::endUI);
			} else {
		 ...
		 */
		public void getToBuffer(FrameBuffer target, boolean clear) {
			if (currBuffer == null) {
				throw new IllegalStateException("currently no buffer bound");
			}
