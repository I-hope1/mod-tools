...
import static modtools.ui.windows.bytecode.BytecodeCanvas.*;

/**
 * fork from MapObjectivesDialog
 *
 * @see mindustry.editor.MapObjectivesDialog
 */
public class JavaBytecodeWindow extends Window {
	public    BytecodeCanvas               canvas;
	protected Cons<Seq<BytecodeObjective>> out = arr -> {};
	...
	static {
		...
		setProvider(Class.class, (type, cons) -> cons.get(Object.class));
		setInterpreter(Class.class, (cont, name, type, field, remover, indexer, get, set) -> {
			final Class[] obj = {get.get()};
			getInterpreter(String.class).build(
			 cont, name, new TypeInfo(String.class),
			 field, remover, indexer,
			 () -> obj[0].getName(),
			 res -> {
				 ValueType value = null;
				 try {
					 value = ValueType.valueOf("_" + res);
				 } catch (Throwable ignored) {}
				 if (value != ValueType._object && value != null) {
					 obj[0] = value.type;
				 } else obj[0] = Kit.classOrNull(res);
				 set.get(obj[0]);
			 }
			);
		});
		...
		setProvider(BytecodeObjective.class, (type, cons) -> new FollowWindow("@add") {{
			cont.pane(p -> {
				...
				int i = 0;
				for (var gen : BytecodeObjectives.allObjectiveTypes) {
					var obj = gen.get();
		...
		/* setProvider(ObjectiveMarker.class, (type, cons) -> new FollowWindow("@add") {{
			cont.pane(p -> {
				p.background(Tex.button);
				p.marginRight(14f);
				p.defaults().size(195f, 56f);
				...
				int i = 0;
				for (var gen : BytecodeObjectives.allMarkerTypes) {
					var marker = gen.get();
		...
		for (var obj : BytecodeObjectives.allObjectiveTypes) setInterpreter(obj.get().getClass(), defaultInterpreter());
		// for (var mark : BytecodeObjectives.allMarkerTypes) setInterpreter(mark.get().getClass(), defaultInterpreter());
	...
	public JavaBytecodeWindow() {
		super("@editor.objectives", 400, 400, true);
		clearChildren();
		margin(0f);

		// update(() -> setBounds(0,0,width,height));
		pane(t -> t.stack(
		 new Image(Styles.black5),
		 canvas = new BytecodeCanvas(),
		 new Table() {{
			 buttons.defaults().size(160f, 64f).pad(2f);
			 buttons.button("@back", Icon.left, JavaBytecodeWindow.this::hide);
			 buttons.button("@add", Icon.add, () -> getProvider(BytecodeObjective.class).get(new TypeInfo(BytecodeObjective.class), canvas::query));
			 ...
			 row().add().grow();
			 row().add(buttons).fill().touchable(Touchable.enabled);
			 // addCloseListener();
		 }}
		).grow().pad(0f).margin(0f)).grow().get().setScrollingDisabled(true, true);
		titleTable.toFront();
	...
	public void show(Seq<BytecodeObjective> objectives, Cons<Seq<BytecodeObjective>> out) {
		this.out = out;
	...
	public static <T extends UnlockableContent> void showContentSelect(@Nullable ContentType type, Cons<T> cons,
																																		 Boolf<T> check) {
		Window dialog = new FollowWindow("");
	...
	public static <T> void setInterpreter(Class<? extends Annotation> anno, Class<T> type,
																				FieldInterpreter<? super T> interpreter) {
