...
public class cRulesDialog extends CustomRulesDialog {
    public Rules rules;
    private Table main;
...
//	    super();
    ...
    public static void main(){
        Events.run(EventType.ClientLoadEvent.class, () -> {
			try {
				Field gamef = CustomGameDialog.class.getDeclaredField("dialog");
				gamef.setAccessible(true);
				var playDialog = (MapPlayDialog) gamef.get(ui.custom);
				Field playf = MapPlayDialog.class.getDeclaredField("dialog");
				playf.setAccessible(true);
				playf.set(playDialog, new cRulesDialog());
			} catch (Exception ex) {
				ui.showException(ex);
			}
		});
        Log.info("change rules successfully");
    }
}
