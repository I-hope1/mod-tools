 ...
 * @author Nathan Sweet
 */
public class MyTextField extends Element implements Disableable {
	...
	protected StringBuilder undoText = new StringBuilder();
	...
	public void addInputDialog() {
		...
		inputDialogListener = tapped(() -> {
			...
			input.multiline = this instanceof MyTextArea;
			input.accepted = text -> {
				clearText();
				appendText(text);
				change();
	...
	public int delete(boolean fireChangeEvent) {
		...
		text.delete(minIndex, minIndex);
        /*StringBuilder newText = (minIndex > 0 ? text.substring(0, minIndex) : "")
        + (maxIndex < text.length() ? text.substring(maxIndex) : "");*/
		if (fireChangeEvent)
			changeText(last, text);
		else
			text = last;
		clearSelection();
		return minIndex;
	...
	public void next(boolean up) {
		...
		MyTextField current = this;
		while (true) {
			current.parent.localToStageCoordinates(tmp1.set(x, y));
			MyTextField textField = ((MyTextField) current).findNextTextField(stage.getElements(), null, tmp2, tmp1, up);
	...
	private MyTextField findNextTextField(Seq<Element> elements, MyTextField best, Vec2 bestCoords, Vec2 currentCoords, boolean up) {
		for (int i = 0, n = elements.size; i < n; i++) {
			Element element = elements.get(i);
			if (element == this || !element.visible) continue;
			if (element instanceof MyTextField) {
				MyTextField textField = (MyTextField) element;
	 ...
	 */
	public boolean changeText(StringBuilder oldText, StringBuilder newText) {
	...
	public interface TextFieldListener {
		void keyTyped(MyTextField textField, char c);
	}
	...
	public interface TextFieldFilter {
		...
		boolean acceptChar(MyTextField textField, char c);
	}

	public interface TextFieldValidator {
		boolean valid(StringBuilder text);
	}
